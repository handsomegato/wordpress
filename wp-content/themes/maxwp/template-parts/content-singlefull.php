                ...
                if ( !(maxwp_get_option('hide_thumbnail')) ) {
                    if ( !(maxwp_get_option('hide_thumbnail_single')) ) {
                        if ( maxwp_get_option('thumbnail_link') == 'no' ) {
                            the_post_thumbnail('maxwp-large-image', array('class' => 'maxwp-post-thumbnail-single'));
                        } else { ?>
                            <a href="<?php echo esc_url( get_permalink() ); ?>" title="<?php /* translators: %s: post title. */ echo esc_attr( sprintf( __( 'Permanent Link to %s', 'maxwp' ), the_title_attribute( 'echo=0' ) ) ); ?>"><?php the_post_thumbnail('maxwp-large-image', array('class' => 'maxwp-post-thumbnail-single')); ?></a>
                <?php   }
                    }
                }
            }
